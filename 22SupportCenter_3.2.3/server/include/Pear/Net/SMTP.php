...
class Net_SMTP
{
     ...
     * @since   1.0
     */
    function Net_SMTP($host = null, $port = null, $localhost = null, $use_tls = false)
    {
        if (defined('SMTP_AUTH_METHODS') && strlen(SMTP_AUTH_METHODS)) {
            $methods = str_replace(' ', '', SMTP_AUTH_METHODS);
    ...
    {
        if (empty($this->_esmtp['AUTH']) || $this->tls) {
            if (version_compare(PHP_VERSION, '5.1.0', '>=') || $this->tls) {

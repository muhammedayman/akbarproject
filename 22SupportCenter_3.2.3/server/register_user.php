...
$paramsService->setField('name', get_magic_quotes_gpc() ? stripslashes($_REQUEST['name']) : $_REQUEST['name']);
...
$passwordAgain = get_magic_quotes_gpc() ? stripslashes($_REQUEST['password_again']) : $_REQUEST['password_again'];
$paramsService->setField('password', md5($password));
$paramsService->set('plain_password', $password);

if ($password == $passwordAgain) {
    if (strlen($_REQUEST['email'])) {
        if ($oService->callService('Users', 'registerNewUser', $paramsService)) {
            $params['message'] = $state->lang->get('manualUserRegistrationSuccessful');
        } else {
    ...
    $params['message'] = $state->lang->get('passwordsDoesntMatch');
}

ob_start();
include(SERVER_PATH . 'templates/knowledgebase/register.inc.php');

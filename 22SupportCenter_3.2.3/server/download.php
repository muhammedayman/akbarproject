...
if( !ini_get('safe_mode') ){
	...
	$downloader = QUnit::newObj('App_Service_Attachments');
	$downloader->state = $state;
	$downloader->response = $response;
	
	if (strlen($_GET['download_type'])) {
		$downloader->downloadFile($_GET['attachment_id'], $_GET['download_type']);
	} else {
		$downloader->downloadFile($_GET['attachment_id']);
	}
?>

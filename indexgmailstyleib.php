...
$username = 'aymanprojecttest@gmail.com';
$password = 'akbar123*';
/* try to connect */
...
<?php	
        foreach($emails as $email_number) { 
                ...
                /* output the email header information */
				 ...
				 $output.= '<span class="name" style="min-width: 120px; display: inline-block;">'.$overview[0]->from.'</span>';
				if (isset($overview[0]->subject))
{
	
   $output.= '<span class="" style="padding-right:180px;">&nbsp;'.$overview[0]->subject.'<br></span> ';
}
				
             
          $output.= '<span class="text-muted" style="font-size: 11px;">'.$message.'</span> ';
					//"- The files will be available for a limited time only unless 
				//	you are a sendspace Premium member."
				
				
			   $time = $overview[0]->date;
			   $time = date("H:i:s",strtotime($time));
                $output.= '<span class="badge">on '.$overview[0]->date.'</span>';
                $output.= '</div>';
